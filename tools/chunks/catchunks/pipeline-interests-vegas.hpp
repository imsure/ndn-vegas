...
#ifndef NDN_TOOLS_CHUNKS_CATCHUNKS_PIPELINE_INTERESTS_VEGAS_HPP
#define NDN_TOOLS_CHUNKS_CATCHUNKS_PIPELINE_INTERESTS_VEGAS_HPP
...
class PipelineInterestsVegasOptions : public Options
{
public:
  explicit
  PipelineInterestsVegasOptions(const Options& options = Options())
    : Options(options)
    , initCwnd(1.0)
...
public:
  ...
  double vegasBeta;
  double vegasGamma;
  Milliseconds minVegasEpochDuration;
};
 ...
 */
class PipelineInterestsVegas : public PipelineInterests
{
public:
  typedef PipelineInterestsVegasOptions Options;

public:
  /**
   * @brief create a PipelineInterestsVegas service
   *
   * Configures the pipelining service without specifying the retrieval namespace. After this
   * configuration the method run must be called to start the Pipeline.
   */
  PipelineInterestsVegas(Face& face, RttEstimator& rttEstimator,
                         RateEstimator& rateEstimator,
                         const Options& options = Options());

  ~PipelineInterestsVegas();
   ...
   */
  signal::Signal<PipelineInterestsVegas, Milliseconds, double> afterCwndChange;
...
private:
  ...
  /**
   * @brief vegas congestion avoidance
   */
  void
  vegasUpdate();
...
PUBLIC_WITH_TESTS_ELSE_PROTECTED:
  ...
  // Milliseconds m_vegasEpoch;
  Milliseconds m_baseRtt;
  Milliseconds m_minRttInVegasEpoch;
  uint64_t m_nReceivedInVegasEpoch; ///<
  Milliseconds m_vegasEpochDuration;
  double m_cwndInVegasEpoch;
};
...
using vegas::PipelineInterestsVegas;

} // namespace chunks
} // namespace ndn

#endif // NDN_TOOLS_CHUNKS_CATCHUNKS_PIPELINE_INTERESTS_VEGAS_HPP
